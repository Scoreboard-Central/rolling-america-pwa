...
while True:
    ...
    discon = []
    for s in states:
        if len(get_components(new_grid, s)) > 1:
            discon.append(s)
            
    if not discon:
        print("All states continuous!")
        break
        
    print(f"Iteration {iterations} disconnected: {discon}")
    for s in discon:
            ...
            while pq:
                ...
                for dr, dc in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                    ...
                    if 0 <= nr < new_H and 0 <= nc < new_W:
                        ...
                        # Add penalty to discourage breaking diagonals
                        if cell_state != s and cell_state != '..':
                            # penalize breaking thin walls
                            step_cost += 2
