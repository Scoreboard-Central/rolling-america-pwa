...
# Valid neighbor lists based on test_map_rules.py outputs or game board
    ...
    while pq:
        ...
        for dr, dc in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            ...
            if 0 <= nr < H and 0 <= nc < W:
                ...
                if cell_state == '..':
                    step_cost = 1000 # Avoid empty space
                elif cell_state == state:
                    step_cost = 0
                elif cell_state in valid_ns:
                    step_cost = 1
                else:
                    step_cost = 500 # Strongly avoid invalid neighbors
...
for s in discon:
    ...
    if len(comps) > 1:
        ...
        for r, c in path:
            grid[r][c] = s
        print(f"Bridged {s} with path length {len(path)}")

joined = ",\n    ".join('"' + " ".join(row) + '"' for row in grid)
new_code = code[:m.start(1)] + "\n    " + joined + "\n" + code[m.end(1):]
with open('make_map.py', 'w') as f: f.write(new_code)
print("Auto-bridged remaining states safely!")
